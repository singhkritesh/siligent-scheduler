...
install_linux() {
  ...
  if siligent_have apt-get; then
    if [[ "$needs_packages" == "true" ]]; then
      ...
      run_as_root apt-get install -y ca-certificates curl openssl
      if ! siligent_have docker; then
    ...
    [[ "$needs_packages" == "false" ]] \
      || run_as_root dnf install -y ca-certificates curl openssl moby-engine docker-compose-plugin
  elif siligent_have pacman; then
    [[ "$needs_packages" == "false" ]] \
      || run_as_root pacman -Syu --needed --noconfirm ca-certificates curl openssl docker docker-compose
  ...
  if ! docker info >/dev/null 2>&1 && [[ "$current_uid" -ne 0 ]]; then
    run_as_root usermod -aG docker "${USER:?USER is required}"
